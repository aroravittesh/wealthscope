...
export class AuthService {
  ...
  // ✅ Clean mock users (NO passwords stored)
  private mockUsers: Map<string, { user: User }> = new Map([
    ['test@example.com', {
      user: {
        id: '1',
  ...
  register(email: string, password: string, fullName: string): Observable<AuthResponse> {
    ...
    // ✅ Store only user (no password)
    this.mockUsers.set(email, { user: newUser });
  ...
  login(email: string, password: string): Observable<AuthResponse> {
    // Create user if not exists (mock behavior)
    if (!this.mockUsers.has(email)) {
      const newUser: User = {
